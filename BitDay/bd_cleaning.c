...
#define BB_CLEANING_ERRS_N 23              // Number of errors.
...
static int bb_cleaning_data_add(char *s, size_t s_len, size_t address_len, struct bb_cleaning_data *pcd, struct bb_cleaning_queue ***ppcurrent, 
    struct bd_cleaning_date **pdates);
...
/*  Allocates memory and adds date to list.  */
inline static int bg_cleaning_add_date(struct bd_cleaning_date **pdates, int year, int month, int date);
                    ...
                    else
                    {
                        ...
                        if (!err_code)
                        {
                            ...
                            *p = '\0';
                            ...
                            err_code = bb_cleaning_data_add(p2, (p - p2), address_len, *ppcd, &pcurrent, pdates);
                            *p = c;
    ...
    struct bd_cleaning_date **pdates)
{
    ...
    *(s + address_len) = c;
    ...
    if (!err_code)
        bb_get_transaction(s, s_len, address_len, &node->pt, &pcd->min_date_year, &pcd->min_date_month, &pcd->min_date_date, pdates);
            ...
            if (*p)
            {
                ...
                else
                {
                    ...
                    if (!err_code)
                    {
                        ...
                        *p = '\0';
                        //puts(p2);
                        err_code = bb_get_transaction_from_string(p2, ppt, pyear, pmonth, pdate, pdates);
                        *p = c;
        ...
        **ppt = (struct bb_cleaning_transaction *) calloc(1, sizeof(struct bb_cleaning_transaction));
        ...
        else
        {
            ...
            *ppt = &(**ppt)->next;
            ...
            if (pdates)
                err_code = bg_cleaning_add_date(pdates, year, month, date);
        }
    }
...
int bd_cleaning_get_addresses_n(struct bb_cleaning_data *pcd, struct bd_cleaning_date *dates, struct bb_error *perr)
{
    ...
    struct bd_cleaning_date *pd;
    const struct bb_cleaning_transaction *pt;
    _Bool *dates_flags, *pb;
    size_t n;
    int m, err_code;
    ...
    {
        for (pd = dates, n = 0; pd; pd = pd->next, ++n)
            continue;

        dates_flags = (_Bool *) calloc(n, sizeof(_Bool));
        if (!dates_flags)
            err_code = 1;
        else
        {
            q_pcurrent = &q_first;
            err_code = bb_cleaning_queue_get_item(pcd->first, &q_pcurrent);
            while (q_first && !err_code)
            {
                memset(dates_flags, 0, n);
                ...
                for (pt = q_first->node->trs; pt && !err_code; pt = pt->next)
                {
                    for (m = -1, pd = dates, pb = dates_flags; pd; pd = pd->next, ++pb)
                    {
                        m = bb_cleaning_compare(pd->year, pd->month, pd->date, pt->year, (int) pt->month & 0xff, (int) pt->date & 0xff);
                        if (m >= 0)
                            break;
                    }
...
inline static int bg_cleaning_add_date(struct bd_cleaning_date **pdates, int year, int month, int date)
{
    struct bd_cleaning_date *node, **pcurrent;
    int n, err_code;

    err_code = 0;

    for (n = -1, pcurrent = pdates; *pcurrent; pcurrent = &(*pcurrent)->next)
    {
        n = bb_cleaning_compare(year, month, date, (*pcurrent)->year, (*pcurrent)->month, (*pcurrent)->date);
        if (n <= 0)
            break;
    }
    ...
    if (n)
    {
        node = (struct bd_cleaning_date *) calloc(1, sizeof(struct bd_cleaning_date));
        if (!node)
            err_code = 1;
        else
        {
...
const char * bb_cleaning_error_message(int err_code)
{
    static const char *BB_CLEANING_ERRS[BB_CLEANING_ERRS_N] = { "ОК",                                                           //  0
                                                                ...
                                                                "Ошибка snprintf",                                              // 21
                                                                "Узел содержит дату, отсутствующую в списке дат",               // 22
    };
