...
int bb_write_balances(const char *file_name, const struct bb_m_data *pmd, struct bb_error *perr)
{
    ...
    else
    {
        ...
        {
            //err_code =  ((fprintf(f, "Address;Bitcoins;Dollars;LastDayDollars;Balance;Buy/Sell/Nothing\n") < 0) ? 
            err_code = ((fprintf(f, "Address;valuewalletBTC;valuewallet$;LastDayDollars;tradingbalance;Buy/Sell/Nothing\n") < 0) ? 
                        17 : 
                        bb_write_balance(f, pmd->first));
...
int bb_get_graph_data(struct bb_m_data *pmd, struct bg_data_node *pd, double last_day_rate, struct bb_error *perr)
{
    ...
    {
        memset(pd, 0, sizeof(struct bg_data_node));
        ...
        while (q_first && !err_code)
        {
            ...
            if (node->left)
                err_code = bb_m_tree_get_queue_item(&q_first, node->left);
            if (node->right && !err_code)
                err_code = bb_m_tree_get_queue_item(&q_first, node->right);

            if (!err_code)
            {
                last_day_t_dollars = ((double) node->last_day_satoshi * last_day_rate) / (double) (BB_SATOSHI_PER_BITCOIN / 1000);
                if (node->last_day_satoshi > (int64_t) 0)
                {
                    pd->buy_n++;

                    pd->btc_buy_n += node->last_day_satoshi;
                    //pd->dol_sel_n += llround(node->last_day_dollars * (double) 1000);
                    pd->dol_sel_n += last_day_t_dollars;
                }
                else if (node->last_day_satoshi < (int64_t) 0)
                {
                    pd->sel_n++;

                    pd->btc_sel_n -= node->last_day_satoshi;
                    //pd->dol_buy_n -= llround(node->last_day_dollars * (double) 1000);
                    pd->dol_buy_n -= last_day_t_dollars;
                }

                if (node->balance > +BB_D)
                {
                    pd->win_n++;
                    ...
                    if (node->last_day_satoshi > (int64_t) 0)
                    {
                        ...
                        pd->win_btc_buy_n += node->last_day_satoshi;
                        //pd->win_dol_sel_n += llround(node->last_day_dollars * (double) 1000);
                        pd->win_dol_sel_n += last_day_t_dollars;
                    }
                    else if (node->last_day_satoshi < (int64_t) 0)
                    {
                        pd->win_sel_n++;

                        pd->win_btc_sel_n -= node->last_day_satoshi;
                        //pd->win_dol_buy_n -= llround(node->last_day_dollars * (double) 1000);
                        pd->win_dol_buy_n -= last_day_t_dollars;
                    }
                }
                else if (node->balance < -BB_D)
                {
                    pd->los_n++;
                    ...
                    if (node->last_day_satoshi > (int64_t) 0)
                    {
                        ...
                        pd->los_btc_buy_n += node->last_day_satoshi;
                        //pd->los_dol_sel_n += llround(node->last_day_dollars * (double) 1000);
                        pd->los_dol_sel_n += last_day_t_dollars;
                    }
                    else if (node->last_day_satoshi < (int64_t) 0)
                    {
                        pd->los_sel_n++;

                        pd->los_btc_sel_n -= node->last_day_satoshi;
                        //pd->los_dol_buy_n -= llround(node->last_day_dollars * (double) 1000);
                        pd->los_dol_buy_n -= last_day_t_dollars;
                    }
                }
            }
...
/*
        puts("-----");
        puts("buy/sell");
        printf("\tbuy_n %zu\n", pd->buy_n);                       // Number of buyers.
        printf("\tsel_n %zu\n", pd->sel_n);                       // Number of sellers.
        puts("win/lose");
        printf("\twin_n %zu\n", pd->win_n);                       // Number of winners.
        printf("\tlos_n %zu\n", pd->los_n);                       // Number of losers.
        puts("win/lose && buy/sell");
        printf("\twin_buy_n %zu\n", pd->win_buy_n);               // Number of winners-buyers.
        printf("\twin_sel_n %zu\n", pd->win_sel_n);               // Number of winners-sellers.
        printf("\tlos_buy_n %zu\n", pd->los_buy_n);               // Number of losers-buyers.
        printf("\tlos_sel_n %zu\n", pd->los_sel_n);               // Number of losers-sellers.
        puts("last day dollars");
        printf("\tdol_buy_n %ld\n", pd->dol_buy_n);               // Dollars * 1000, bought on last day.
        printf("\tdol_sel_n %ld\n", pd->dol_sel_n);               // Dollars * 1000, sold on last day.
        puts("last day bitcoins");
        printf("\tbtc_buy_n %ld\n", pd->btc_buy_n);               // Bitcoins in satoshi (* 100 000 000), bought on last day.
        printf("\tbtc_sel_n %ld\n", pd->btc_sel_n);               // Bitcoins in satoshi (* 100 000 000), sold on last day.
        puts("win/lose && buy/sell && last day dollars/last day bitcoins");
        printf("\twin_btc_buy_n %ld\n", pd->win_btc_buy_n);       // Bitcoins in satoshi (* 100 000 000), bought on last day by winners.
        printf("\twin_btc_sel_n %ld\n", pd->win_btc_sel_n);       // Bitcoins in satoshi (* 100 000 000), sold on last day by winners.
        printf("\twin_dol_buy_n %ld\n", pd->win_dol_buy_n);       // Dollars * 1000, bought on last day by winners.
        printf("\twin_dol_sel_n %ld\n", pd->win_dol_sel_n);       // Dollars * 1000, sold on last day by winners.
        printf("\tlos_btc_buy_n %ld\n", pd->los_btc_buy_n);       // Bitcoins in satoshi (* 100 000 000), bought on last day by losers.
        printf("\tlos_btc_sel_n %ld\n", pd->los_btc_sel_n);       // Bitcoins in satoshi (* 100 000 000), sold on last day by losers.
        printf("\tlos_dol_buy_n %ld\n", pd->los_dol_buy_n);       // Dollars * 1000, bought on last day by losers.
        printf("\tlos_dol_sel_n %ld\n", pd->los_dol_sel_n);       // Dollars * 1000, sold on last day by losers.
*/
    }
